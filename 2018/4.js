const part1 = () => {
    const guardMostSleep = sleepList()[0];
    const timeMapSorted = Object.entries(timeMap(guardMostSleep))
        .map(([key, value]) => ({key, value}))
        .sort((a, b) => b.value - a.value);

    return parseInt(guardMostSleep.key) * parseInt(timeMapSorted[0].key);
};

const part2 = () => {
    const topMap = {};
    sleepList().forEach(sleep => {
        const timeMapSorted = Object.entries(timeMap(sleep))
            .map(([key, value]) => ({key, value}))
            .sort((a, b) => b.value - a.value);
        topMap[sleep.key] = timeMapSorted[0];
    });
    const topMapSorted = Object.entries(topMap)
        .map(([key, value]) => ({key, value}))
        .sort((a, b) => b.value.value - a.value.value);

    return parseInt(topMapSorted[0].key) * parseInt(topMapSorted[0].value.key);
};
    ...
    sleepingRecords = sleepingRecords.map(sleepingRecords => parseSleepingRecord(sleepingRecords))
        ...
        .forEach(sleepingRecord => {
            if (sleepingRecord.id) {
                id = sleepingRecord.id;
            } else if (sleepingRecord.state.startsWith('f')) {
                let object = map[id];
                if (object) {
                    object.sleepDate = sleepingRecord.date;
                    object.sleepDates.push(sleepingRecord.date);
                } else {
                    object = {sleepDate: sleepingRecord.date};
                    object.sleepDates = [sleepingRecord.date];
                    object.awakesDates = [];
                }
                map[id] = object;
            } else if (sleepingRecord.state.startsWith('w')) {
                const sleepingTime = (sleepingRecord.date - map[id].sleepDate) / 1000 / 60;
                const object = map[id];
                object.total = object.total ? object.total + sleepingTime : sleepingTime;
                map[id] = object;
                object.awakesDates.push(sleepingRecord.date);
            }
        });
...
const parseSleepingRecord = (sleepingRecord) => {
    const date = new Date(sleepingRecord.substring(0, 18)
        .replace('[', '')
        .replace(']', '')
    );
    const state = sleepingRecord.substring(19);
    let id;
    if (state.startsWith('G')) {
        id = parseInt(state.split(' ')[1].replace('#', ''));
    }
    return {
        date, state, id
    }
...
const timeMap = (sleep) => {
    const timeMap = {};
    for (let i = 0; i < sleep.value.sleepDates.length; i++) {
        const sleepDate = sleep.value.sleepDates[i];
        let sleepMinute = sleepDate.getMinutes();
        const awakeDate = sleep.value.awakesDates[i];
        const awakeMinute = awakeDate.getMinutes();

        for (let j = sleepMinute; j < awakeMinute; j++) {
            const value = timeMap[j];
            timeMap[j] = value ? value + 1 : 1;
        }
    }
    return timeMap;
};
